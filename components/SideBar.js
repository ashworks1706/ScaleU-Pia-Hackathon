...
export default function SidebarChat() {
  ...
  return (
    <div
      className={`h-screen flex flex-col relative bg-black backdrop-blur-md border-r border-neutral-700 transition-all duration-150 ${
    ...
    >
      ...
      <button
        ...
        className="w-full px-4 pt-4 text-neutral-400 hover:text-white transition-colors flex items-center justify-end"
      >
        {isExpanded ? (
          <svg
            className="h-6 w-6"
            ...
            {["Homework", "Projects", "Exams"].map((channel) => (
              <li key={channel}>
                <button
                  ...
                  className={`w-full p-2 rounded-lg text-neutral-400 hover:bg-neutral-800 hover:text-white transition-colors flex items-center gap-2 ${
            ...
            {messages[activeChannel].map((message) => (
              <div key={message.id} className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-neutral-300">{message.text}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="p-4 border-t border-neutral-700">
          <div className="flex items-center space-x-2">
            <input
              ...
              className="w-full p-2 rounded-lg bg-black border border-neutral-700 text-white placeholder-neutral-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
              value={newMessage}
